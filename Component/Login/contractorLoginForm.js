...
import { StyleSheet, Text, View , ImageBackground , TextInput , ActivityIndicator, AsyncStorage, ScrollView , TouchableOpacity , Image, Animated} from 'react-native';
import {heightPercentageToDP as hp, widthPercentageToDP as wp} from 'react-native-responsive-screen';
import { Icon } from 'react-native-elements' ;
import axios from 'axios' ;
import ShowBar from '../validations/messageBar' ;
import { connect } from 'react-redux';
import {StoreTokenNumber, StoreUserName, store_Role_Id_And_Name} from '../../Redux/createJob/jobAction' ;
import {server} from '../../Redux/server';
import {messaging} from 'react-native-firebase';

class ContractorLoginForm extends Component{
  constructor(props){
    super(props);
      this.state ={
        email : '' ,
    ...
    try{
      ...
      if(res.data.code === 201) {
        ...
        if(res.data.data.active){
          ...
          this.props.navigation.navigate('EmailVerification' , {emailAddress : this.state.email, role : 'freelancer', screen : 'ContractorLogin', userID : userID});
          this.stopProcessing();
        } 
      }
    }
  ...
  storeUserData = (userName, currentUserId, role, coHost, email ) => {
    obj = {
      role,
      ...
      <ImageBackground source={require('../../assets/Nouvelle_bg.jpg')} style={{flex : 1}} resizeMode={'stretch'}>
        <ScrollView>
          <View style={styles.textEntrycontainer}>
            ...
            <Text style={{fontSize : (23) ,fontFamily : 'Raleway-SemiBold'  , textAlign : 'center' ,  color : '#292929', marginBottom :hp('2%')}}>SIGN IN</Text>
            <Text style={{fontSize : (20), fontFamily : 'Raleway-SemiBold' , textAlign : 'center' ,  color : '#0071bc' , marginBottom :hp('4%')}}>Contractor</Text>    
            ...
            <View style = {styles.rememberMeCheckBox}>
              <TouchableOpacity onPress = {() => this.setState({checked : !this.state.checked ? true : false})} style={{flexDirection : 'row' , alignItems : 'center'}}>
                ...
                <Text style={{ color : '#292929' , fontSize : (15), marginLeft : wp('4%')}}>Keep me logged-in</Text>
              </TouchableOpacity>
          ...
          <View style={styles.alreadyAccount}>
            ...
            <TouchableOpacity onPress={()=>this.props.navigation.navigate('ContractorReg')}>
...
export default connect(mapStateToProps, {StoreTokenNumber, StoreUserName, store_Role_Id_And_Name})(ContractorLoginForm);
