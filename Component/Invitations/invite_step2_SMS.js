...
import { StyleSheet, Text, View, ScrollView, TextInput, FlatList, Image, ImageBackground, TouchableOpacity, Platform, ActivityIndicator} from 'react-native';
...
class Invite_Step2_SMS extends Component{
  constructor(props){
    super(props);
    this.state = {
      name : '',
  ...
  onSend = async() => {
    if(this.state.phoneNum){
      ...
      const res = await axios.post(server+'invite_host', formData, {
  ...
  render(){
    return(
      <ImageBackground source={require('../../assets/invite_contractor_bg.jpg')} style={styles.container}>
        <StatusBar title='INVITATION' isIconDisplay={true} navigation={this.props.navigation} marginValue={hp('0%')}/>
        <ProgressBar stepOneColor='#8CC63E' stepTwoColor='#8CC63E'/>
        <ScrollView>
...
export default connect(mapStateToProps)(Invite_Step2_SMS);
